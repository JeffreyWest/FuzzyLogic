...
//default constructor
AU_UAV_ROS::OTWOWBHtighter::OTWOWBHtighter(void){
    ...
    ourBearingAngle->addTerm(new fl::TriangularTerm("POS", 44.0, 89.0));
    ourBearingAngle->addTerm(new fl::ShoulderTerm("VERYPOS", 45.0, 90.0, false));
    ...
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is NEG and TheirBearingAngle is LOWPOS and PlaneDist is FAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is NEG and TheirBearingAngle is POS and PlaneDist is FAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is NEG and TheirBearingAngle is VERYPOS and PlaneDist is FAR then ChangeInHeading is RIGHT", *engine));
    ...
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is LOWNEG and PlaneDist is FAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is LOWPOS and PlaneDist is FAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is POS and PlaneDist is FAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is VERYPOS and PlaneDist is FAR then ChangeInHeading is LEFT", *engine));
    
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is VERYNEG and PlaneDist is FAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is NEG and PlaneDist is FAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is LOWNEG and PlaneDist is FAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is LOWPOS and PlaneDist is FAR then ChangeInHeading is LEFT", *engine));
    ...
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is POS and TheirBearingAngle is VERYNEG and PlaneDist is FAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is POS and TheirBearingAngle is NEG and PlaneDist is FAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is POS and TheirBearingAngle is LOWNEG and PlaneDist is FAR then ChangeInHeading is LEFT", *engine));
    ...
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is VERYNEG and TheirBearingAngle is LOWNEG and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is VERYNEG and TheirBearingAngle is LOWPOS and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is VERYNEG and TheirBearingAngle is POS and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is VERYNEG and TheirBearingAngle is VERYPOS and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
    ...
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is NEG and TheirBearingAngle is NEG and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is NEG and TheirBearingAngle is LOWNEG and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is NEG and TheirBearingAngle is LOWPOS and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is NEG and TheirBearingAngle is POS and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is NEG and TheirBearingAngle is VERYPOS and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is VERYNEG and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is NEG and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is LOWNEG and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is LOWPOS and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is POS and PlaneDist is VERYFAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWNEG and TheirBearingAngle is VERYPOS and PlaneDist is VERYFAR then ChangeInHeading is LEFT", *engine));
    
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is VERYNEG and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is NEG and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is LOWNEG and PlaneDist is VERYFAR then ChangeInHeading is RIGHT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is LOWPOS and PlaneDist is VERYFAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is POS and PlaneDist is VERYFAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is LOWPOS and TheirBearingAngle is VERYPOS and PlaneDist is VERYFAR then ChangeInHeading is LEFT", *engine));
    
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is POS and TheirBearingAngle is VERYNEG and PlaneDist is VERYFAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is POS and TheirBearingAngle is NEG and PlaneDist is VERYFAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is POS and TheirBearingAngle is LOWNEG and PlaneDist is VERYFAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is POS and TheirBearingAngle is LOWPOS and PlaneDist is VERYFAR then ChangeInHeading is LEFT", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is POS and TheirBearingAngle is POS and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
    ...
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is VERYPOS and TheirBearingAngle is VERYNEG and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is VERYPOS and TheirBearingAngle is NEG and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is VERYPOS and TheirBearingAngle is LOWNEG and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
    block->addRule(new fl::MamdaniRule("if OurBearingAngle is VERYPOS and TheirBearingAngle is LOWPOS and PlaneDist is VERYFAR then ChangeInHeading is NOCHANGE", *engine));
...
double AU_UAV_ROS::OTWOWBHtighter::process(double distance, double ourAngle, double theirAngle){
